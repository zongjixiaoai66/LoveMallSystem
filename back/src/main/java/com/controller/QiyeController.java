...
public class QiyeController {
    ...
    @Autowired
    private QiyeYuyueService qiyeYuyueService;//公益企业捐赠
    @Autowired
    ...
    */
    ...
    public R save(@RequestBody QiyeEntity qiye, HttpServletRequest request){
        ...
        if(qiyeEntity==null){
            ...
            return R.error(511,"账户或者企业手机号已经被使用");
        }
    }
...
//                            qiyeEntity.setQiyeName(data.get(0));                    //企业姓名 要改的
//                            qiyeEntity.setQiyePhone(data.get(0));                    //企业手机号 要改的
...
//                            qiyeEntity.setCreateTime(date);//时间
                        ...
                        if(qiyeEntities_qiyePhone.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [企业手机号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
